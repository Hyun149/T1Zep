...
✅ 주요 작업 내역
1. 스크립트 정비 및 구조 개선
- 전체 스크립트에 기능 설명 주석 추가
- 클래스 책임, 메서드 역할, 흐름 중심 정리
- 튜터님 검토 및 코드 유지보수성 향상 목적
- 사용되지 않는 GameManager.cs 및 meta 파일 제거
→ 싱글톤 구조이나 참조되지 않음

2. 큐브 시스템 개선 (T1Stack)
- 생성된 큐브가 10초 후 자동 파괴되도록 기능 추가
- 이후 30초로 파괴 시간 조정하여 게임 흐름 조절

3. 플레이어 발사 방향 및 회전 구현
- 이동 방향에 따라 캐릭터 좌우 반전(flip) 처리
- 발사 지점 회전(파이어포인트 방향)을 이동 방향과 일치시킴

💡 작업 의도 및 효과
- 주석 기반 문서화를 통해 전체 구조 파악 용이성 향상
- 큐브 자동 파괴로 오브젝트 누적 방지 및 성능 관리
- 발사 방향 정비로 게임 감각 및 피드백 정확도 개선
- 유지보수성 및 리팩토링 기반을 강화하여 향후 기능 확장에 유리한 구조 확보
